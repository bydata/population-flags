...
# Add Kosovo
kosovo <- rnaturalearth::ne_countries(
  country = "Kosovo", scale = 50, returnclass = "sf") %>% 
  mutate(iso_a3 = "KOS", iso_a2 = "XK")
world <- world %>%
  bind_rows(select(kosovo, FID = iso_a2, ISO3_CODE = iso_a3, NAME_ENGL = name, geometry))

# Limit France to mainland
france <- world %>%
  filter(FID == "FR") %>%
  st_crop(
...
pop_latest <- pop_latest %>%
  ...
  add_row(country_name = "Kosovo", country_code_2 = kosovo$iso_a2,
        country_code_3 = kosovo$iso_a3, year = kosovo$pop_year, population = kosovo$pop_est)
...
st_crs(world_dorling)
# world_dorling <- st_transform(world_dorling, "EPSG:4326")
